function res = Classifica_LDA_Kfold(dados,classes,K)    
    ...
    for y=1:K   
        try
            f = lda(dados(Folds(y).treino,:),classes(Folds(y).treino));
            [c,post] = classify(f,dados(Folds(y).teste,:));
            acertos = sum(c == classes(Folds(y).teste));
            %res = res + (100*acertos/length(c));
            res = res + acertos;
        catch
        end
    end
